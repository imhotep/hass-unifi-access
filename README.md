...
While Double-Driveway Mode is enabled, the standard `open` / `close` / `stop` cover controls for that door are hidden, because those actions use `control_cmd` and are the wrong scheme for dual-relay hubs (they only fire the entry relay). The `Open Gate (In)` / `Open Gate (Out)` buttons replace them and send `entry_method=in` / `entry_method=out` instead. When Double-Driveway Mode is off, the normal cover controls remain available.
