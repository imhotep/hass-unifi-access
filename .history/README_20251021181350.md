...
- It should find all of your doors and users and add the following entities:

## Door Entities (for each door)
...
## User Management Entities (for each user)
- User Status Switch (switch). Enable or disable user access through Home Assistant interface
- Each user appears as a separate switch entity allowing individual control
...
- Select `Verify SSL certificate` only if you have a valid SSL certificate. For example: If your Unifi Access API server is behind a reverse proxy. Selecting this will fail otherwise.
- Select `Use polling` if your Unifi Access version is < 1.90. Default is to use websockets for instantaneous updates and more features.
- It should find all of your doors and users and add the entities described above.
...
This integration provides comprehensive user management capabilities for UniFi Access users:

### User Switch Entities
- Each user in your UniFi Access system appears as a switch entity in Home Assistant
- Switch ON = User enabled/active, Switch OFF = User disabled/inactive
- Changes are reflected in real-time and synced with the UniFi Access system

### Home Assistant Services

The integration provides three services for automation and advanced control:

#### `unifi_access.enable_user`
Enables access for a specific user
- **user_id**: The ID of the user to enable
- **config_entry_id**: The configuration entry ID for the UniFi Access integration

#### `unifi_access.disable_user`  
Disables access for a specific user
- **user_id**: The ID of the user to disable
- **config_entry_id**: The configuration entry ID for the UniFi Access integration

#### `unifi_access.update_user_pin`
Updates the PIN code for a specific user
- **user_id**: The ID of the user to update
- **pin**: The new PIN code (leave empty to remove PIN)
- **config_entry_id**: The configuration entry ID for the UniFi Access integration

### User Management Examples

#### Enable/Disable User via Automation
```yaml
# Enable user when motion is detected
alias: Enable Guest User on Motion
trigger:
  - platform: state
    entity_id: binary_sensor.entrance_motion
    to: 'on'
action:
  - service: unifi_access.enable_user
    data:
...
```yaml
# Disable user at night
alias: Disable Guest Access at Night
trigger:
  - platform: time
    at: "22:00:00"
action:
  - service: unifi_access.disable_user
    data:
      user_id: "guest-user-id-here" 
      config_entry_id: "your-config-entry-id"
```

#### Update User PIN via Automation
```yaml
# Set temporary PIN for guest
alias: Set Guest PIN
action:
  - service: unifi_access.update_user_pin
    data:
      user_id: "guest-user-id-here"
      pin: "1234"
      config_entry_id: "your-config-entry-id"
